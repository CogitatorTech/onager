...
//!
//! PageRank, Degree, Betweenness, Closeness, Eigenvector, Katz, Harmonic centrality.
...
pub fn compute_pagerank(
    ...
    if directed {
        ...
        Ok(PageRankResult { node_ids: result_nodes, ranks: result_ranks })
    } else {
...
pub fn compute_degree(src: &[i64], dst: &[i64], directed: bool) -> Result<DegreeResult> {
    ...
    if directed {
        ...
        let in_deg = in_degree_centrality(&graph).map_err(|e| OnagerError::GraphError(e.to_string()))?;
        let out_deg = out_degree_centrality(&graph).map_err(|e| OnagerError::GraphError(e.to_string()))?;
        let mut result_nodes = Vec::with_capacity(node_set.len());
        ...
        Ok(DegreeResult { node_ids: result_nodes, in_degrees: result_in, out_degrees: result_out })
    } else {
        let mut graph: Graph<i64, f64> = Graph::new();
        for &node in src.iter().chain(dst.iter()) {
            if !node_set.contains_key(&node) {
        ...
        let deg = in_degree_centrality(&graph).map_err(|e| OnagerError::GraphError(e.to_string()))?;
        let mut result_nodes = Vec::with_capacity(node_set.len());
        ...
        Ok(DegreeResult { node_ids: result_nodes, in_degrees: result_deg.clone(), out_degrees: result_deg })
    }
}
...
pub fn compute_betweenness(src: &[i64], dst: &[i64], normalized: bool) -> Result<BetweennessResult> {
    if src.len() != dst.len() {
        return Err(OnagerError::InvalidArgument("src and dst arrays must have same length".to_string()));
    }
    if src.is_empty() {
        return Err(OnagerError::InvalidArgument("Cannot compute betweenness on empty graph".to_string()));
    }
    ...
    Ok(BetweennessResult { node_ids: result_nodes, centralities: result_cent })
}
...
pub fn compute_closeness(src: &[i64], dst: &[i64]) -> Result<ClosenessResult> {
    ...
    if src.is_empty() {
        return Err(OnagerError::InvalidArgument("Cannot compute closeness on empty graph".to_string()));
    }
    ...
    let centralities = closeness_centrality(&graph).map_err(|e| OnagerError::GraphError(e.to_string()))?;
    let mut result_nodes = Vec::with_capacity(node_set.len());
    let mut result_cent = Vec::with_capacity(node_set.len());
    for (ext_id, int_id) in &node_set {
        result_nodes.push(*ext_id);
    ...
    Ok(ClosenessResult { node_ids: result_nodes, centralities: result_cent })
}
...
pub fn compute_eigenvector(src: &[i64], dst: &[i64], max_iter: usize, tolerance: f64) -> Result<EigenvectorResult> {
    if src.len() != dst.len() {
        return Err(OnagerError::InvalidArgument("src and dst arrays must have same length".to_string()));
    }
    if src.is_empty() {
        return Err(OnagerError::InvalidArgument("Cannot compute eigenvector on empty graph".to_string()));
    }
    ...
    Ok(EigenvectorResult { node_ids: result_nodes, centralities: result_cent })
}
...
pub fn compute_katz(src: &[i64], dst: &[i64], alpha: f64, max_iter: usize, tolerance: f64) -> Result<KatzResult> {
    if src.len() != dst.len() {
        return Err(OnagerError::InvalidArgument("src and dst arrays must have same length".to_string()));
    }
    if src.is_empty() {
        return Err(OnagerError::InvalidArgument("Cannot compute Katz on empty graph".to_string()));
    }
    ...
    Ok(KatzResult { node_ids: result_nodes, centralities: result_cent })
}
...
pub fn compute_harmonic(src: &[i64], dst: &[i64]) -> Result<HarmonicResult> {
    ...
    if src.is_empty() {
        return Err(OnagerError::InvalidArgument("Cannot compute harmonic on empty graph".to_string()));
    }
    ...
    let centralities = harmonic_centrality(&graph).map_err(|e| OnagerError::GraphError(e.to_string()))?;
    let mut result_nodes = Vec::with_capacity(node_set.len());
    let mut result_cent = Vec::with_capacity(node_set.len());
    for (ext_id, int_id) in &node_set {
        result_nodes.push(*ext_id);
    ...
    Ok(HarmonicResult { node_ids: result_nodes, centralities: result_cent })
}
