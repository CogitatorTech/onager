...
pub extern "C" fn onager_get_version() -> *mut c_char {
    CString::new(VERSION).map(|s| s.into_raw()).unwrap_or(std::ptr::null_mut())
}
...
pub extern "C" fn onager_create_graph(name: *const c_char, directed: bool) -> i32 {
    ...
    let name = match unsafe { CStr::from_ptr(name) }.to_str() {
        Ok(s) => s,
        Err(_) => { set_last_error("Invalid UTF-8 in graph name"); return -1; }
    };
...
#[no_mangle]
pub extern "C" fn onager_add_edge(graph_name: *const c_char, src: i64, dst: i64, weight: f64) -> i32 {
    clear_last_error();
    let name = match unsafe { CStr::from_ptr(graph_name) }.to_str() {
        Ok(s) => s,
        Err(_) => { set_last_error("Invalid UTF-8 in graph name"); return -1; }
